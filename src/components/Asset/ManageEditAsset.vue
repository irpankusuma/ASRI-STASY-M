      ...
      <v-stepper-items>
        <v-stepper-content step="0">
          ...
          <v-card>
            <v-card-text>
              ...
              <v-flex xs12>                                                    
                <v-autocomplete clearable label="Type (*)" required v-model="selectedType" disabled
              ...
              <v-flex xs12>                                                    
                <v-autocomplete clearable label="Assign Color (*)" required v-model="selectedColor" disabled
        ...
        <v-stepper-content step="1">
          ...
          <v-card>
            <v-card-text>
              ...
              <v-flex xs12>                                                    
                <v-text-field outline v-model="selectedAssetDate" label="Asset Date" type="date" disabled></v-text-field>                                                  
              </v-flex>
              <v-flex xs12>                    
        ...
        <v-stepper-content step="2">
          ...
          <v-card>
            <v-card-text>
              <v-flex xs12>                                                    
                <v-autocomplete clearable label="Assign User (*)" required v-model="selectedUser" disabled
                    outline item-text="AssetUserName" item-value="IDX_M_AssetUser" :items="listsUser"    
                ></v-autocomplete>                                              
              </v-flex>
              <v-flex xs12>                                                    
                <v-autocomplete clearable label="Assign Management (*)" required v-model="selectedManagement" disabled
                    outline item-text="AssetManagementName" item-value="IDX_M_AssetManagement" :items="listsManagement"    
                ></v-autocomplete>                                              
              </v-flex>
              <v-flex xs12>                                                    
                <v-autocomplete clearable label="Assign Company (*)" required v-model="selectedCompany" disabled
                    outline item-text="CompanyName" item-value="IDX_M_Company" :items="listsCompany"    
                ></v-autocomplete>                                              
              </v-flex>
              <v-flex xs12>                                                    
                <v-autocomplete clearable label="Assign Location (*)" required v-model="selectedLocation" disabled
                    outline item-text="AssetLocationName" item-value="IDX_M_AssetLocation" :items="listsLocation"    
                ></v-autocomplete>                                              
              </v-flex>
              <v-flex xs12>                                                    
                <v-autocomplete clearable label="Assign Status (*)" required v-model="selectedStatus" disabled
        ...
        <v-stepper-content step="3">
          <v-toolbar flat dense>
            ...
            <v-toolbar-items>
              <v-btn @click="tabs--"><v-icon left color="cyan">keyboard_arrow_left</v-icon>BACK</v-btn>
              <v-btn @click="update()"><v-icon left color="green lighten-2">save</v-icon>UPDATE</v-btn>
            </v-toolbar-items>
          </v-toolbar>
          <v-card>
            <v-card-text>
...
export default {
  ...
  data(){
    return {
      ...
      selectedPO:null,
      selectedPODate:this.$moment('1900-01-01').format("YYYY-MM-DD"),
  ...
  methods:{
    ...
    loadList(idx) {
      var id = { IDX_M_Asset: idx };
      this.axios
        .post("ManageAsset/loadAsset_By_IDXAsset", id)
        .then(res => {
          let data = res['Table'][0];
          this.selectedManagement = data.IDX_M_AssetManagement;
          this.selectedCompany = data.IDX_M_Company;
          this.selectedUser = data.IDX_M_AssetUser;
          this.selectedLocation = data.IDX_M_AssetLocation;
          this.selectedStatus = data.IDX_M_AssetStatus;
          this.selectedPO = data.PONo;
          this.selectedPODate = this.$moment(data.PODate).format('YYYY-MM-DD');
          this.isConnectASBS = data.isConnectASBSPO;
          this.selectedRemarks = data.Remarks;
          this.selectedType = data.IDX_M_AssetType;
          this.selectedTypeModel = data.AssetTypeModelName;
          this.selectedBrand = data.AssetBrandName;
          this.selectedColor = data.IDX_M_AssetColor;
          this.selectedSize = data.AssetSizeName;
          this.selectedCurrency = data.Currency;
          this.selectedUnitPrice = data.UnitPrice;
          this.selectedAssetDate = this.$moment(data.AssetDate).format('YYYY-MM-DD');
          this.selectedVendor = data.IDX_M_Vendor;
          this.selectedMaterial = data.IDX_M_Item;
          this.selectedAssetType = data.AssetType;
          this.selectedSpesification = data.Spesification;
          this.selectedSerialNumber = data.SerialNumber;
          this.selectedReceiveDate = this.$moment(data.ReceiveDate).format('YYYY-MM-DD');
          this.selectedDeliveryDate = this.$moment(data.DeliveryDate).format('YYYY-MM-DD');
        })
        .catch(error => {
    ...
    update() {
      ...
      this.client('Asset/updateAsset',id,(res) => this.$emit('close',res['Table'][0].StatusCSS == 'success' ? false : true));
      this.tabs = 0;
    },
    checkPO(){
      this.poDialog = true;
