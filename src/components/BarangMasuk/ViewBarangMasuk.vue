              ...
              <template v-slot:items="props">
                <tr :style="props.item.isOpname ? 'color:yellow' : null">
                  <td class="text-xs-center">
                    <v-icon v-if="props.item.isOpname">locked</v-icon>
                    {{ props.item.InventoryStockCode }}
                  </td>
    ...
    loadList(){
      ...
      this.axios
        ...
        .then(response => {
          ...
          this.selectedRequestDate = this.$moment(r.InRequestFormDate).format('DD MM YYYY | HH:mm');
          this.selectedRequestCompany = r.CompanyName;
          this.selectedPONo = r.PONo;
          this.selectedPODate = this.$moment(r.PODate).format('DD MM YYYY | HH:mm');;
