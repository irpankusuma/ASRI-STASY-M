<template>
  <v-container fluid>
    ...
    <v-toolbar dark>
      <v-toolbar-items>
        <v-btn @click="newRequestDialog =! newRequestDialog" v-if="isNew"><v-icon left color="lime">add</v-icon>BRAND BARU</v-btn>
      </v-toolbar-items>
    ...
    >
      <v-card dark>
        ...
        <v-card-text>
          <v-text-field v-model="editedName" label="Brand" solo-inverted />
        </v-card-text>
        <v-divider></v-divider>
    ...
    >
      <v-card dark>
        ...
        <v-card-text>
          <v-text-field v-model="inputedBrandName" label="Brand" solo-inverted />
        </v-card-text>
        <v-divider></v-divider>
        <v-card-actions class="justify-center">
...
export default {
  ...
  data(){
    return {
      headers: [
        ...
        { text:'NAME',align:'left',sortable:true,value:'InventoryBrandName' },
        { text:'COUNT',align:'left',sortable:true,value:'Count' },
      ],
      lists:[],
      additionals:[],
      selectedKeyword:null,
      inputedBrandName:null,
      editedIDX:null,
      editedName:null,
  ...
  methods:{
    ...
    actionMenu(p){
      let code = p.code;
      let id = p.id;
      let arr = this.lists.filter(k => { return k.IDX_M_InventoryBrand == id })
      
      switch (code){
        case 'isUpdate':
          this.editRequestDialog = true;
          this.editedIDX = id;
          this.editedName = arr[0].InventoryBrandName;
        break;
        case 'isEnable':
    ...
    toSave(p){
      let id = {
        ...
        InventoryBrandName: this.inputedBrandName
      }

      this.client("Master/saveBrand",id,() => {
        this.loadList(); 
    ...
    toUpdate(p){
      let id = {
        Session_ID: this.$session.get('SESSION_ID'),
        IDX_M_InventoryBrand: this.editedIDX,
        InventoryBrandName: this.editedName
      }

      this.client("Master/updateBrand",id,() => {
        this.loadList(); 
    ...
    toEnable(a,b){
      let id = {
        Session_ID: this.$session.get('SESSION_ID'),
        IDX_M_InventoryBrand:b
      }
      this.client(a ? "Master/enableBrand" : "Master/disableBrand",id,() => this.loadList());
    },
    toDelete(i){
      let id = {
        Session_ID: this.$session.get('SESSION_ID'),
        IDX_M_InventoryBrand:i
      }
      this.client("Master/deleteBrand",id,() => this.loadList());
    },
    close(p){
